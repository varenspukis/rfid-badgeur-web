...
function doDefaultSelect(name) {
    var defaultSelect = "SELECT * FROM '" + name + "' GROUP BY Nom,Prenom ORDER BY DATE DESC";
...
function renderQueryCSV(query) {
    ...
    while (sel.step()) {
        if (!addedColums) {
            addedColums = true;
            var csv = "#" + query.replace(/(\r\n|\n|\r)/gm, " ");  
            csv = csv + "\n" + sel.getColumnNames().toString();
        }
    ...
    export_link.display = "none";
    export_link.href = "data:text/plain;charset=utf-8," + encodeURIComponent(csv);
    export_link.download = "export.csv";
